...
Updates data.json in the current directory
...
def find_latest_page():
    ...
    today = datetime.now()
    # Stats publishes around 15th-18th of each month
    for m in range(12):
        ...
        # Just return the most recent known URL pattern
    return None
...
def main():
    ...
    # Try the known April 2026 URL first (which we know works)
    # In production, this would use find_latest_page()
    ...
    # If second hand parsing failed, try end marker
    ...
    data_path = os.path.join(script_dir, '..', 'data.json') if 'scrape' in os.path.basename(__file__) else os.path.join(script_dir, 'data.json')
    
    with open('data.json', 'w', encoding='utf-8') as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    
    abs_path = os.path.abspath('data.json')
    print(f"\n✅ 数据已保存到: {abs_path}")
    
    # Preview key cities
    key_cities = ['杭州', '大连', '昆明', '哈尔滨']
    print(f"\n📈 重点关注城市 ({period}):")
    for c in new_house:
    ...
    print()
    print("---")
    print("Tip: 将此脚本设为定时任务，每月中旬自动更新数据。")
    print("示例 cron: 0 10 15-18 * * cd /path/to/repo && python3 scrape_70city.py")
